    ...
    def _fill_sale_purchase_dashboard_data(self, dashboard_data):
        ...
        for journal in sale_purchase_journals:
            ...
            dashboard_data[journal.id].update({
                ...
                'number_waiting': number_waiting,
                'number_late': number_late,
                'sum_draft_usd': currency.format(sum_draft * currency_id_dif.rate),
                'sum_draft': currency.format(sum_draft),  # sign is already handled by the SQL query

                'sum_waiting_usd': currency.format(sum_waiting * sum_waiting * currency_id_dif.rate * (1 if journal.type == 'sale' else -1)),
                # 'sum_waiting': currency.format(sum_waiting * currency_id_dif.rate),
                'sum_waiting': currency.format(sum_waiting * (1 if journal.type == 'sale' else -1)),
                'sum_late_usd': currency.format(sum_late * currency_id_dif.rate * (1 if journal.type == 'sale' else -1)),
