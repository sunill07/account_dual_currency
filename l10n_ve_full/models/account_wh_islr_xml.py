    ...
    def _xml(self):
        ...
        for ixwd_id in self.ids:
            ...
            if local_ids:
                ...
                xml_lines = []
            company_vat = rp_obj._find_accounting_partner(wh_brw.company_id.partner_id).rif[0] if rp_obj._find_accounting_partner(wh_brw.company_id.partner_id).rif else False
            company_vat = company_vat.replace("-", "") if company_vat else False
            company_vat1 = wh_brw.company_id.partner_id.rif
            company_vat1 = company_vat1.replace("-", "") if company_vat1 else False
